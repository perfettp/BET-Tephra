...
import bet.service.suite

def bulk_suite(start_date, end_date, interval=None):
    if not interval:
    ...
    while sample_time <= end_date:
        bet.service.suite.main(['suite', '-c', 'etc/bet.cfg',
                                '--no_sampling', '--obs_time',
                                sample_time.strftime("%Y%m%d_%H%M%S")])
        sample_time += interval
        print "Done with sampling time {}\n\n".format(
            sample_time.strftime("%Y%m%d_%H%M%S"))


start_date_str = "20160901_012000"
end_date_str = "20160930_132000"
start_date = datetime.strptime(start_date_str, "%Y%m%d_%H%M%S")
end_date = datetime.strptime(end_date_str, "%Y%m%d_%H%M%S")

bulk_suite(start_date, end_date, interval=timedelta(hours=12))
